...
def enumerate_subdomains(domain: str, wordlist: list = None, wordlist_file: str = None) -> list:
    """
    Attempt to discover subdomains using a wordlist.
    Supports both a hardcoded list and an external .txt file.
    ...
    if wordlist_file:
        try:
            ...
            print(f"[*] Loaded {len(wordlist)} entries from {wordlist_file}")
        except FileNotFoundError:
            print(f"[-] Wordlist file not found: {wordlist_file}, using default")
            wordlist = None

    # Fall back to default if no wordlist provided
    if wordlist is None:
        wordlist = [
        ...
        print(f"[*] Using default wordlist ({len(wordlist)} entries)")
...
def run_osint(domain: str, wordlist_file: str = None) -> dict:
    """
    ...
    results = {
        ...
        "subdomains": enumerate_subdomains(domain, wordlist_file=wordlist_file),
    }
